(function ($) {
	...
	// Keep a handle so we don't re-init
	var fogLoader = null;
	var windowOn = $(window);

	function initFogLoader() {
		var $root = $('#fog-loader');
		if (!$root.length || fogLoader) return; // no element or already inited

		var $circle = $root.find('.vanta-circle');

		// Init Vanta effect once
		fogLoader = VANTA.FOG({
			el: $circle[0],
			THREE: window.THREE,
			highlightColor: 0xffffff,
			midtoneColor: 0x1d9bd1,
			lowlightColor: 0x1d9bd1,
			baseColor: 0xffffff,
	...
	function showLoader() {
		initFogLoader();
		$('#fog-loader').stop(true, true).fadeIn(200);
		$('body').css('overflow', 'hidden'); // optional: lock scroll
	}

	function hideLoader() {
		$('#fog-loader').stop(true, true).fadeOut(200, function () {
			$('body').delay(350).css({ 'overflow': 'visible' });
		});
		$('#preloader').delay(350).fadeOut('slow');
	}

	// Show immediately, hide when everything is fully loaded
	showLoader();
	...
	new Swiper('.brands-swiper', {
		loop: true,
		speed: 5000, // Higher = slower scroll speed
		autoplay: {
			delay: 0, // No delay between slides
			disableOnInteraction: false
		},
		freeMode: true, // Continuous scrolling mode
		...
		breakpoints: {
			...
			1200: { slidesPerView: 8, spaceBetween: 32 }
		}
	});
